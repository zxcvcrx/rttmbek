<div align="center">

<img src="https://raw.githubusercontent.com/rajput2107/rajput2107/master/Assets/PC.gif" width="200"/>

# Assalawma Aleykum! 👋

### I'm Uzakbergenov — Network & System Administrator from Uzbekistan,Nukus

</div>

---

## 🧩 About Me

- 🖥️ I am a **Network and System Administrator** with experience in configuring and maintaining computer systems.
- 🛠 I specialize in **printers setup, maintenance and troubleshooting**.
- 🌐 Network devices setup: routers, switches, access points.
- 📡 Strong knowledge in **Windows OS management, Wi-Fi and LAN configuration**.
- 📝 Experienced in **Office programs** (Word, Excel, PowerPoint).
- 🎯 I also work with **Telegram Ads**, channel and bot management.
- 🎨 Basic Graphic Editing (Canva, basic Photoshop tools).

---

## 🛠 Skills & Technologies

<div>
  
### 🔧 System & Network Administration  
- Windows OS Management  
- Network Configuration (LAN / Wi-Fi)  
- Router & Switch Setup  
- User & Permission Management  
- BIOS / Drivers Troubleshooting  

### 🖨 Printer Service  
- Printer installation  
- Driver setup  
- Troubleshooting & maintenance  

### 📊 Office Programs  
- MS Word  
- MS Excel  
- PowerPoint  

### 📣 Digital Marketing  
- Telegram ADS  
- Channel Optimization  

### 🎨 Graphic Editing  
- Basic Photoshop  
- Canva  

</div>

---

## 🚀 Tools I Use

<div align="center">
<img src="https://skillicons.dev/icons?i=windows,linux,powershell,github,figma,kerio" />
</div>

## 📬 Connect with me

<div align="center">

<a href="https://t.me/@m511g" target="_blank">
  <img src="https://skillicons.dev/icons?i=telegram" width="40"/>
</a>

<a href="https://linkedin.com/in/nukuss" target="_blank">
  <img src="https://skillicons.dev/icons?i=linkedin" width="40"/>
</a>

<a href="mailto:dtununus@gmail.com">
  <img src="https://skillicons.dev/icons?i=gmail" width="40"/>
</a>

<a href="https://github.com/zxcvcrx" target="_blank">
  <img src="https://skillicons.dev/icons?i=github" width="40"/>
</a>
<a href="https://instagram.com/xcvrzm" target="_blank">
  <img src="https://skillicons.dev/icons?i=instagram" width="40" />
</a>
</div>

- Phone: +998 90 705 87 76
- Telegram: **@m511g**
- Email: **dtunukus@gmail.com**

---
